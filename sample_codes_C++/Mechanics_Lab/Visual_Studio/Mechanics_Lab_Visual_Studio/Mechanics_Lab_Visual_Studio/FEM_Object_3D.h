...
public:
	...
	int Solver();
	int Obtain_Local_Assemblies(const std::string& Step_Name, const int& Inc);
	int Obtain_Global_Assembly_RHS(const std::string& Step_Name, const int& Inc);
	int Obtain_Local_Assembly(const std::string& Step_Name, const int& Inc, const int& Ele_Index, std::vector <int> Connectivity_Local);
	int Obtain_Local_Normal_Assembly(struct_Local_Para& Local_Para);
	int ApplyBCs(const std::string& Step_Name, const int& Inc);
	int Solve_Equation();
