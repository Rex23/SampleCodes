...
int FEM_Object_3D::Solver()
{
	...
	for (std::map < std::string, std::vector <double> >::const_iterator iter1 = Step.StepName_Static_Parameters.begin(); 
		 iter1 != Step.StepName_Static_Parameters.end(); iter1++)
	{
...
{
	for (int m = 0; m < Mesh.Connectivities.size(); m++){
		Obtain_Local_Assembly(Inc, m+1, Mesh.Connectivities[m]);
	}
...
int FEM_Object_3D::Obtain_Local_Assembly(const int& Inc, const int& Ele_Index, std::vector <int> Connectivity_Local)
{
	...
	Local_Para.Ele_Type = Mesh.Connectivities_Type[Ele_Index - 1];
	...
	for (int m1 = 0; m1 < Local_Para.Nnode; m1++) {
		for (int m2 = 0; m2 < 3; m2++) {
			Local_Para.Coords[m1][m2] = Mesh.Nodes[Connectivity_Local[m1] - 1][m2];
		}
	}

	Local_Para.Material_Name = Model->Connectivities_Material_Name[Ele_Index - 1];

	Material.Obtain_MaterialName_Properties(Local_Para.Material_Name, Local_Para.CC);
...
int FEM_Object_3D::Obtain_Local_Normal_Assembly(struct_Local_Para& Local_Para)
{
	if (Local_Para.Ele_Type == "C3D8") {
		Local_Para.C3D8 -> Obtain_C3D8_Local_Normal_Assembly(Local_Para.Coords);
	}
...
int FEM_Object_3D::Read_Input_File(const std::string& FileName)
{
	...
	while (getline(infile, tmp)) {
		...
		if (tmp[0] == '*' && tmp[1] != '*') {
			...
			if (MatchKeyword(Terms[0].c_str(), "*NODE")) {
				...
				while (true) {
					...
					if ((tmp2[0] == '*' && tmp2[1] != '*') ) {
						...
						Mesh.Connectivities.push_back(row_Temp);
						Mesh.Connectivities_Type.push_back(Ele_Type);
			...
			//**************************Material**************************
				...
				Material.Material_Name_Type[Material_Name] = Material_Type;

				//std::cout << "sfsdfsfdMaterial Name Type: " << Material_Name << ", " << Material_Type << std::endl;

				Material.Material_Name_Properties[Material_Name] = Material_Properties;
			}
			//************************************************************
			...
			//**************************Set**************************
				...
				if (!gen_keyword_bool) {
					...
					Mesh.Element_Set[elset_name] = ele_set_temp;
				...
				if (!gen_keyword_bool) {
					...
					Mesh.Node_Set[nset_name] = nset_temp;
				}
				else {
					std::vector <int> nset_temp;
					while (true) {
			...
			//**************************Section**************************
				...
				Section.SectionSet_Material_Type[elset] = Material_Type;
			}
			//***********************************************************
			...
			//**************************Surface*****************************
				...
				Surface.SurfaceName_Type[Surface_Name] = Surface_Type;
				...
				while (true) {
					...
					if ((tmp2[0] == '*' && tmp2[1] != '*') ) {
						...
						Surface.SurfaceName_Sets[Surface_Name] = Set_Surface;
					}
				}
			}
			//**************************************************************
			...
			//**************************Load*****************************
				...
				while (true) {
					...
					if ((tmp2[0] == '*' && tmp2[1] != '*') ) {
						...
						Load.DsloadSurface_LoadType[Surface] = LoadType;
						Load.DsloadSurface_Magnitude[Surface] = Magnitude;

						if (!Step_Name.empty()) {
							Step.StepName_Dsloads[Step_Name].push_back(Surface);
						}
						else {
			...
			//**************************Boundary*****************************
				...
				while (true) {
					...
					if ((tmp2[0] == '*' && tmp2[1] != '*') ) {
						...
						Boundary.BoundarySet_Directions[Terms[0]] = Directions;
						Boundary.BoundarySet_Magnitude[Terms[0]] = Magnitude;
						//std::cout << "Set Name: " << Terms[0] << ", " << "Directions: " << Directions[0] << ", " << Directions[1] << ", " << Magnitude << std::endl;
					
						if (!Step_Name.empty()) {
							Step.StepName_Boundaries[Step_Name].push_back(Terms[0]);
						}
						else {
					...
					if ((tmp2[0] == '*' && tmp2[1] != '*') ) {
						...
						if (!Step_Name.empty()) {
							...
							for (int mm = 0; mm < Terms.size(); mm++) {
								Step.StepName_Static_Parameters[Step_Name].push_back( atof(Terms[mm].c_str()) );
	...
	Model -> ObtainElementsMaterialName(Mesh, Material, Section);
