...
int Class_Model::ObtainNodesAppliedBCs()
{
	//std::map < std::string, std::vector <std::string> > StepName_Boundaries;

	for (auto iter0 = StepName_Boundaries.begin(); iter0 != StepName_Boundaries.end(); iter0++) {

		std::string StepName = iter0->first;

		std::vector <std::string> BoundarySets = iter0->second;

		for (auto m1 = 0; m1 < BoundarySets.size(); m1++) {
			//std::cout << "Test BoundarySets: " << BoundarySets[m1] << std::endl;

			std::string SetName = BoundarySets[m1];
			//std::map < std::string, std::vector <int> > Node_Set;
			//if it is not a number
			std::vector <int> Nodes_Set;
			if (isdigit(SetName[0])) {
				Nodes_Set.push_back( atoi(SetName.c_str()) );
				//std::cout << "Test Number: " << atoi(SetName.c_str()) << std::endl;
			}
			else {
				Nodes_Set = Node_Set.find(SetName)->second;
			}

			//std::map < std::string, std::vector <int> > BoundarySet_Directions;
			//std::map < std::string, double > BoundarySet_Magnitude;

			std::vector <int> Directions = BoundarySet_Directions.find(SetName)->second;
			double Magnitude = BoundarySet_Magnitude.find(SetName)->second;

			for (auto m = 0; m < Nodes_Set.size(); m++) {
				for (auto m1 = Directions.begin(); m1 != Directions.end(); m1++) {
					Nodes_BC[StepName][Nodes_Set[m]][*m1] = Magnitude;
				}
			}
		}
	}
...
{
	for (auto iter0 = Connectivities_Pressure.begin(); iter0 != Connectivities_Pressure.end(); iter0++) {

		std::string Step_Name = iter0->first;

		std::map <int, std::map <int, double> > Connectivities_Pressure_Each_Step = iter0->second;

		for (auto iter1 = Connectivities_Pressure_Each_Step.begin(); iter1 != Connectivities_Pressure_Each_Step.end(); iter1++) {
				...
				for (auto mm = 0; mm < Nodes_Index.size(); mm++) {
					for (auto mm1 = 0; mm1 < 3; mm1++) {
						if (Global_Force[mm1] != 0.0E0) {
							Nodes_Force[Step_Name][Connectivities[Ele_Index - 1][Nodes_Index[mm] - 1]][mm1 + 1] += Global_Force[mm1] / Nodes_Index.size();
						}
					}
				}
			}
...
int Class_Model::ObtainElementsSurfacePressure()
{
	...
	for (auto iter0 = StepName_Dsloads.begin(); iter0 != StepName_Dsloads.end(); iter0++) {

		std::string StepName = iter0->first;
		std::vector <std::string> Dsloads_SurfaceNames = iter0->second;

		for (auto uu = 0; uu < Dsloads_SurfaceNames.size(); uu++) {

			std::string Surface_Name = Dsloads_SurfaceNames[uu];
			double Pressure_Magnitude = DsloadSurface_Magnitude.find(Surface_Name)->second;
			...
			for (auto iter1 = Sets_Direction.begin(); iter1 != Sets_Direction.end(); iter1++) {
				...
				if (Surface_Type == "ELEMENT") {
					...
					for (auto m = 0; m < Elements.size(); m++) {
						if (Direction == "S1") {
							Connectivities_Pressure[StepName][Elements[m]][1] = Pressure_Magnitude;
						}
						else if (Direction == "S2") {
							Connectivities_Pressure[StepName][Elements[m]][2] = Pressure_Magnitude;
						}
						else if (Direction == "S3") {
							Connectivities_Pressure[StepName][Elements[m]][3] = Pressure_Magnitude;
						}
						else if (Direction == "S4") {
							Connectivities_Pressure[StepName][Elements[m]][4] = Pressure_Magnitude;
						}
						else if (Direction == "S5") {
							Connectivities_Pressure[StepName][Elements[m]][5] = Pressure_Magnitude;
						}
						else if (Direction == "S6") {
							Connectivities_Pressure[StepName][Elements[m]][6] = Pressure_Magnitude;
						}
					}
...
int Class_Model::ObtainElementsMaterialName()
{
	...
	for (auto iter1 = SectionSet_Material_Type.begin();
		...
		for (auto m1 = 0; m1 < Ele_Set.size(); m1++) {
